 ...
 *   (classic path) + mermaid/packages/mermaid/src/rendering-util/rendering-elements/shapes/hexagon.ts
 ...
 *   Hand-drawn (hexagon.ts): upstream's hand-drawn branch builds a hexagon path via
 *     `createHexagonPathD(0, 0, w, h, m)` and rough-sketches it with `rc.path(pathData, options)`,
 *     then translates the group. SSG's CLASSIC hexagon, however, is built from its OWN six vertices
 *     (top-left/top-right/right-point/bottom-right/bottom-left/left-point, insetting the pointed ends
 *     by `config.height * 0.25`), emitted as a moveTo/lineTo×5/close <path>. To keep hand-drawn
 *     CONSISTENT with SSG's own classic geometry (same shape + size), the hand-drawn branch rough-
 *     sketches SSG's OWN six hexagon vertices via `rough.svg().polygon(Vector(points…), opts)`
 *     (Chip 8 `RoughSVG.polygon`) rather than porting `createHexagonPathD` + `rc.path`. This is the
 *     exact analogue of the 9d diamond decision (SSG rough-polygons its own classic rhombus vertices
 *     instead of upstream's `createDecisionBoxPathD` at `s = w + h`). Options come from
 *     `HandDrawnShapeStyles.userNodeOverrides(node, {})` (seed from `config.handDrawnSeed`); the
 *     returned immutable `SvgElement` is grafted via `HandDrawnShapes.graftElement` with the
 *     `node-shape` class + inline style. Label + Intersect.polygon are unchanged. Classic rendering
 *     is byte-identical.
 ...
 *     `Point` the classic path + intersect use; the coordinates are the same six SSG vertices.
 *
 * upstream-commit: 2cfdd1620 (hexagon.ts classic) / 56a2762 (hexagon.ts hand-drawn, ISS-1204)
 */
package ssg
package mermaid
package render
...
object HexagonShape {
  ...
  def render(parent: SvgBuilder, config: ShapeConfig): ShapeResult = {
    ...
    if (config.look == "handDrawn") {
      // hexagon.ts: the `node.look === 'handDrawn'` branch. Instead of the classic sharp <path>,
      // rough-sketch SSG's OWN six hexagon vertices (the same `points` the classic path uses) via
