...
final class WhereFilterSuite extends munit.FunSuite {
  ...
  // "jekyll where: numeric coercion via parseSortInput" is in src/test/scala-jvm-native,
  // WhereFilterJvmNativeSuite: on Scala.js a whole-number Double cannot be told from an integer.
