...
/** The one test of liqp's ReadmeSamplesTest.java whose filter answers a whole-number Double.
  *
  * JVM and Scala Native only: on Scala.js a whole-number Double cannot be told from an integer, so `15.0` renders as `15`. The other tests are in the shared ReadmeSamplesSuite.
  */
final class ReadmeSamplesJvmNativeSuite extends munit.FunSuite {
