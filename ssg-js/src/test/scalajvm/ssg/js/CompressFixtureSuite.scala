...
final class CompressFixtureSuite extends munit.FunSuite {
  ...
  // submodule is not checked out (e.g. CI with submodules:false), fixtureDir is None and the fixture tests
  // below are not registered at all — these fixtures require the external submodule.
  ...
  private def requireFixtures(): java.nio.file.Path =
    // Only ever called from tests registered under `if (fixtureDir.isDefined)`, so the submodule is present.
    fixtureDir.getOrElse(
      throw new IllegalStateException("Terser fixtures required but submodule absent — test should not have registered")
    )
  ...
  // -----------------------------------------------------------------------
  // Fixture tests — registered only when the original-src/terser submodule is present (fixtureDir.isDefined).
  // When the submodule is not checked out (e.g. CI with submodules:false) these tests are simply absent.
  // -----------------------------------------------------------------------

  if (fixtureDir.isDefined) {
