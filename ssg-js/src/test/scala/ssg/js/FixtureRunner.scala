...
object FixtureRunner {
  ...
  /** Per-fixture timeout (ms). */
  private val FixtureTimeoutMs = 2000L

  /** Run a single fixture. */
  def run(fixture: CompressFixture): FixtureResult = {
    // Run with timeout to prevent hangs on edge-case inputs
    val future = java.util.concurrent.CompletableFuture.supplyAsync(() => runUnsafe(fixture))
    try {
      future.get(FixtureTimeoutMs, java.util.concurrent.TimeUnit.MILLISECONDS)
    } catch {
      case _: java.util.concurrent.TimeoutException =>
        future.cancel(true)
        FixtureResult(passed = false, actual = "", message = "Error: timeout")
      case e: Exception =>
