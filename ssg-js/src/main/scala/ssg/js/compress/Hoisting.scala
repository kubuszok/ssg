...
object Hoisting {
  ...
  /** Convert variable definitions to a sequence of assignments.
    *
    * Each definition with a value becomes `name = value`. Definitions without
    * values are skipped. Returns null if no assignments are generated.
    *
    * This updates the SymbolDef's `eliminated` and `replaced` counters and
    * sets `fixed = false` when `reduce_vars` is enabled.
    */
  ...
  /** Get all parameter names from a lambda, flattening destructuring patterns.
    *
    * For simple parameters, returns the symbol itself. For destructuring
    * patterns, returns all symbols contained within.
    */
  ...
  /** Hoist `var` declarations and function declarations to the top of a scope.
    *
    * When `hoist_vars` is enabled and there are multiple `var` statements,
    * all variable declarations are collected, moved to the top, and their
    * initializers are converted to assignments in-place.
    *
    * When `hoist_funs` is enabled, function declarations are moved to the
    * top of the scope (after directives but before other statements).
    *
            ...
            case ss: AstSimpleStatement =>
              ss.body match {
                case assign: AstAssign
                    if assign.operator == "=" && assign.left != null =>
  ...
  /** Hoist properties from constant object literals into individual variables.
    ...
    *
    * Then each property access `obj.prop` is replaced with a new variable
    * `obj_prop`, and the original object is replaced with individual var
  ...
  def hoistProperties(self: AstScope, compressor: CompressorLike): AstScope = {
    ...
    val topRetainFn: SymbolDef => Boolean =
      if (self.isInstanceOf[AstToplevel]) {
        d => compressor.topRetain(d)
      } else {
        _ => false
      }
    ...
    hoister = new TreeTransformer(before = (node, descend) => {
      node match {
            ...
            case symbol: AstSymbol
              ...
              if (d != null) {
                ...
                if (eligible) {
                  ...
                  if (fixedVal != null && varDef.value != null &&
  ...
  private final case class SpliceMarker(nodes: ArrayBuffer[AstVarDef]) extends AstNode {
  ...
  private def makeUniqueName(scope: AstScope, tentativeName: String, source: AstSymbol): String = {
    ...
    var candidate = name
    var counter = 0
    while (ScopeAnalysis.conflictingDef(scope, candidate) ||
