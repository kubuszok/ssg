...
object SassCalculation {
  ...
  /** Creates an `abs()` calculation with the given argument.
    *
    * Note: The deprecation warning for percentage units is deferred until
    * EvaluationContext is available.
    */
  ...
  def clamp(minArg: Any, valueArg: Nullable[Any] = Nullable.Null, maxArg: Nullable[Any] = Nullable.Null): Value = {
    ...
    (simplifiedMin, simplifiedValue, simplifiedMax) match {
      case (sMin: SassNumber, sv, sm) =>
        (sv, sm) match {
          case (sValue: SassNumber, sMax: SassNumber)
              if sMin.hasCompatibleUnits(sValue) && sMin.hasCompatibleUnits(sMax) =>
  ...
  def atan2(y: Any, x: Nullable[Any]): Value = {
    ...
    (simplifiedY, simplifiedX) match {
      case (yNum: SassNumber, xNum: SassNumber)
  ...
  def roundInternal(
    ...
    (simplifiedStrategy, simplifiedNumberOrStep, simplifiedStep) match {
      ...
      case (number: SassNumber, _, _)
          if simplifiedNumberOrStep.isEmpty && simplifiedStep.isEmpty && inLegacySassFunction.isDefined =>
        warn.foreach(w => w(
      ...
      case (number: SassNumber, stepVal: SassNumber, _)
          if simplifiedStep.isEmpty && !number.hasCompatibleUnits(stepVal) =>
      ...
      case (strategy: SassString, rest: SassString, _)
          if _isRoundingStrategy(strategy.text) && simplifiedStep.isEmpty =>
        new SassCalculation("round", List(strategy, rest))

      // round(strategy, non-null-non-number, null)
      case (strategy: SassString, _, _)
          if _isRoundingStrategy(strategy.text) && simplifiedNumberOrStep.isDefined && simplifiedStep.isEmpty =>
        throw SassScriptException("If strategy is not null, step is required.")

      // round(strategy, null, null)
      case (strategy: SassString, _, _)
          if _isRoundingStrategy(strategy.text) && simplifiedNumberOrStep.isEmpty && simplifiedStep.isEmpty =>
            ...
            case (l: SassNumber, r: SassNumber) =>
              ...
              if (compatible) {
                ...
                if (fuzzyLessThan(r.value, 0)) {
                  ...
                  val flippedOp = if (operator == CalculationOperator.Plus) CalculationOperator.Minus
