...
object AstNode {
  ...
  /** Creates an [AstNode] that doesn't have any data and whose span is
    * generated by [callback].
    *
    * A number of APIs take AstNodes instead of spans because computing spans
  ...
  private final class FakeAstNode(callback: () => FileSpan) extends AstNode {
