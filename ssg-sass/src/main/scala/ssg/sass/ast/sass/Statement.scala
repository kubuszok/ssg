...
/** A [Statement] that can have child statements.
  *
  * @param children the child statements, or empty if this statement has no body
  */
...
/** An abstract class for callables (functions or mixins) declared in user code.
  *
  * @param originalName the callable's original name
  * @param parameters   the declared parameters
  * @param children     the body statements
  * @param span         the source span
  * @param comment      the comment immediately preceding this declaration
  */
...
/** An `@at-root` rule. This moves its contents "up" the tree through
  * parent nodes.
  *
  * @param query the query specifying which statements to move through
  * @param span  the source span
  */
...
/** A `@content` rule. Used in a mixin to include statement-level content
  * passed by the caller.
  *
  * @param arguments the arguments passed to this `@content` rule
  * @param span      the source span
  */
...
/** A declaration (that is, a `name: value` pair).
  ...
  * @param parsedAsSassScript whether the value was parsed as SassScript
  */
...
/** An `@each` rule. Iterates over values in a list or map.
  *
  * @param variables the variables assigned for each iteration
  * @param list      the expression whose value this iterates through
  * @param span      the source span
  */
...
/** An `@error` rule. Emits an error and stops execution.
  *
  * @param expression the expression to evaluate for the error message
  * @param span       the source span
  */
...
/** An `@extend` rule. Gives one selector all the styling of another.
  *
  * @param selector   the interpolation for the selector that will be extended
  * @param span       the source span
  * @param isOptional whether this is an optional extension
  */
...
/** A `@for` rule. Iterates a set number of times.
  *
  * @param variable    the name of the variable that will contain the index
  * @param from        the expression for the start index
  * @param to          the expression for the end index
  * @param span        the source span
  * @param isExclusive whether [to] is exclusive
  */
final class ForRule(
  val variable: String,
  val from: Expression,
...
/** A `@forward` rule.
  *
  * @param url                       the URI of the module to forward
  * @param span                      the source span
  * @param prefix                    the prefix to add to member names, or empty
  * @param shownMixinsAndFunctions   set of shown mixin/function names, or empty
  * @param shownVariables            set of shown variable names, or empty
  * @param hiddenMixinsAndFunctions  set of hidden mixin/function names, or empty
  * @param hiddenVariables           set of hidden variable names, or empty
  * @param configuration             variable assignments to configure loaded modules
  */
...
) extends Statement with SassDependency {
...
/** A function declaration. Declares a function invoked using normal CSS
  * function syntax.
  *
  * @param originalName the function name (original underscores)
  * @param parameters   the declared parameters
  * @param span         the source span
  * @param comment      the comment immediately preceding this declaration
  */
...
/** An `@if` rule. Conditionally executes a block of code.
  *
  * @param clauses    the `@if` and `@else if` clauses
  * @param span       the source span
  * @param lastClause the final `@else` clause, or empty
  */
final class IfRule(
  ...
  override def toString: String = {
    val result = clauses.zipWithIndex.map { case (clause, index) =>
...
/** An `@import` rule.
  *
  * @param imports the imports imported by this statement
  * @param span    the source span
  */
...
/** A mixin invocation.
  *
  * @param originalName the original name of the mixin being invoked
  * @param arguments    the arguments to pass to the mixin
  * @param span         the source span
  * @param namespace    the namespace of the mixin, or empty
  * @param content      the content block, or empty
  */
...
) extends Statement with CallableInvocation with SassReference {
...
/** A loud CSS-style comment.
  *
  * @param text the interpolated text of this comment, including comment characters
  */
...
/** A `@media` rule.
  *
  * @param query the query that determines on which platforms the styles are in effect
  * @param span  the source span
  */
...
/** A mixin declaration. Declares a mixin invoked using `@include`.
  *
  * @param originalName the mixin name (original underscores)
  * @param parameters   the declared parameters
  * @param span         the source span
  * @param comment      the comment immediately preceding this declaration
  */
...
/** A `@return` rule. Exits from the current function body with a return value.
  *
  * @param expression the value to return from this function
  * @param span       the source span
  */
...
/** A silent Sass-style comment.
  *
  * @param text the text of this comment, including comment characters
  * @param span the source span
  */
final class SilentComment(
  ...
  /** The subset of lines marked as documentation comments (beginning with `///`).
    * Returns empty when there is no documentation comment.
    */
...
/** A style rule. Applies style declarations to elements that match a given
  * selector.
  *
  * @param selector       the selector (unparsed interpolation), or empty
  * @param parsedSelector the pre-parsed selector, or empty
  * @param span           the source span
  */
...
/** A Sass stylesheet. This is the root Sass node. It contains top-level
  * statements.
  *
  * @param span              the source span
  * @param plainCss          whether this was parsed from a plain CSS stylesheet
  * @param parseTimeWarnings warnings discovered while parsing
  * @param globalVariables   normalized global variable names -> definition spans
  */
  ...
  private def _collectUses(stmts: List[Statement]): List[UseRule] = boundary {
    ...
    for (child <- stmts) {
      child match {
        ...
        case _: ForwardRule | _: SilentComment | _: LoudComment |
             _: VariableDeclaration                              => // allowed between @use/@forward
  ...
  private def _collectForwards(stmts: List[Statement]): List[ForwardRule] = boundary {
    ...
    for (child <- stmts) {
      child match {
        ...
        case _: UseRule | _: SilentComment | _: LoudComment |
             _: VariableDeclaration                              => // allowed between @use/@forward
        case _                                                   => break(buffer.toList)
      }
    }
...
/** A `@supports` rule.
  *
  * @param condition the condition that selects what browsers this rule targets
  * @param span      the source span
  */
...
/** A `@use` rule.
  *
  * @param url           the URI of the module to use
  * @param namespace     the namespace for members, or empty for no namespace
  * @param span          the source span
  * @param configuration variable assignments to configure the loaded module
  */
final class UseRule(
  val url: URI,
...
/** A variable declaration. Defines or sets a variable.
  *
  * @param name       the name of the variable, with underscores converted to hyphens
  * @param expression the value the variable is being assigned to
  * @param span       the source span
  * @param namespace  the namespace of the variable, or empty
  * @param isGuarded  whether this is a guarded assignment
  * @param isGlobal   whether this is a global assignment
  * @param comment    the comment immediately preceding this declaration
  */
...
) extends Statement with SassDeclaration {
  require(
...
/** A `@while` rule. Repeatedly executes a block of code as long as a
  * statement evaluates to `true`.
  *
  * @param condition the condition that determines whether the block executes
  * @param span      the source span
  */
