...
class Extension(
     ...
     * `Nullable.empty` if it can apply within any context.
     */
...
/** A selector that's extending another selector, such as `A` in
 * `A {@extend B}`.
 */
...
object MergedExtension {
   ...
   * contexts.
   *
   * Throws an [IllegalArgumentException] if [left] and [right] don't have the
