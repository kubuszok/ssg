 ...
 *
 * These functions aren't private methods on [ExtensionStore] because they
 * also need to be accessible from elsewhere in the codebase. In addition,
 * they aren't instance methods on other objects because their APIs aren't
 * a good fit — usually because they deal with raw component lists rather
 * than selector classes, to reduce allocations.
 */
object ExtendFunctions {

  /** Pseudo-selectors that can only meaningfully appear in the first
   * component of a complex selector.
   */
  val RootishPseudoClasses: Set[String] = Set("root", "scope", "host", "host-context")

  /** Returns the contents of a [SelectorList] that matches only elements
   * that are matched by every complex selector in [complexes].
   *
  ...
  /** Returns a [CompoundSelector] that matches only elements matched by
   * both [compound1] and [compound2], or `Nullable.empty` if no such
   * selector exists.
   */
  def unifyCompound(
  ...
  /** Expands "parenthesized selectors" in [complexes] and returns a list of
   * complex selectors, none of which contain parenthesized selectors.
   */
  def weave(
  ...
  /** Returns all pairs of elements where the first element is taken from
   * [list1] and the second from [list2].
   */
