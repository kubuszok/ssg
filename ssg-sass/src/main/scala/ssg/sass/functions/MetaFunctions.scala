...
object MetaFunctions {
  ...
  private val typeOfFn: BuiltInCallable =
    BuiltInCallable.function("type-of", "$value", { args =>
      SassString(typeName(args.head), hasQuotes = false)
    })

  private val inspectFn: BuiltInCallable =
    BuiltInCallable.function("inspect", "$value", { args =>
      SassString(args.head.toString, hasQuotes = false)
    })

  private val featureExistsFn: BuiltInCallable =
    BuiltInCallable.function("feature-exists", "$feature", { _ =>
      // Deprecated; we don't claim to support any features.
      SassBoolean.sassFalse
    })

  private val variableExistsFn: BuiltInCallable =
    BuiltInCallable.function("variable-exists", "$name", { _ =>
      // TODO: requires Environment access; deferred.
      SassBoolean.sassFalse
    })

  private val functionExistsFn: BuiltInCallable =
    BuiltInCallable.function("function-exists", "$name", { args =>
  ...
  val global: List[Callable] = List(
    typeOfFn, inspectFn, featureExistsFn, variableExistsFn, functionExistsFn
  )
