...
object StringFunctions {
  ...
  private val unquoteFn: BuiltInCallable =
    BuiltInCallable.function("unquote", "$string", { args =>
  ...
  private val quoteFn: BuiltInCallable =
    BuiltInCallable.function("quote", "$string", { args =>
      val s = args.head.assertString()
      if (s.hasQuotes) s
  ...
  private val strIndexFn: BuiltInCallable =
    BuiltInCallable.function("str-index", "$string, $substring", { args =>
      val s = args.head.assertString()
  ...
  private val strSliceFn: BuiltInCallable =
    BuiltInCallable.function("str-slice", "$string, $start-at, $end-at: -1", { args =>
      val s = args.head.assertString()
  ...
  val global: List[Callable] = List(
    unquoteFn, quoteFn, strLengthFn, toUpperCaseFn, toLowerCaseFn,
    strInsertFn, strIndexFn, strSliceFn
  )
