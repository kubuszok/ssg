...
/** Built-in selector functions. Minimal text-based implementation — no selector AST parsing. Accepts only [[SassString]] arguments; returns [[SassNull]] when given a list or other non-string value.
  */
object SelectorFunctions {
  ...
  private val selectorUnifyFn: BuiltInCallable =
    BuiltInCallable.function("selector-unify",
                             "$selector1, $selector2",
                             _ =>
    ...
    BuiltInCallable.function(
      ...
      args =>
        if (args.length < 3) SassNull
          ...
          (asText(args(0)), asText(args(1)), asText(args(2))) match {
