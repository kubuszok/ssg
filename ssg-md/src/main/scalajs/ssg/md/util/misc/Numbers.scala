 ...
 *
 * Scala.js: `NumberFormat.parse(String, ParsePosition)` is not implemented by scala-java-locales, so
...
object Numbers {
  ...
  def parse(format: NumberFormat, text: String, pos: ParsePosition): Number = {
    ...
    if (!sawDigit) {
      ...
      val literal     = (if (negative) "-" else "") + digits.substring(0, intDigits) + (if (intDigits < digits.length) "." + digits.substring(intDigits) else "") + (if (exponent != 0) "E" + exponent else "")
      if (integral) {
