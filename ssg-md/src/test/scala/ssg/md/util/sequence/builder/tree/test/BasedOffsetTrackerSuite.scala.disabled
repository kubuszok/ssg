...
final class BasedOffsetTrackerSuite extends munit.FunSuite {
  ...
  private def getInput(input: String): Pair[String, java.lang.Integer] = {
    val pos = input.indexOf("\u299a")
    if (pos >= 0) {
      Pair.of(input.substring(0, pos) + input.substring(pos + 1), java.lang.Integer.valueOf(pos))
    } else {
      Pair.of(input, java.lang.Integer.valueOf(0))
    }
  }
  ...
  private def wrapText(input: String, isEndOffset: Boolean, margin: Int): String = {
    ...
    val sequence1 = BasedSequence.of(info1.getFirst())
    resolveOffset(sequence1, actual, info1.getSecond(), isEndOffset)
  }
    ...
    assertEquals(
      wrapText(
        ...
        { wrapped =>
          ...
          wrapped.splitListEOL(true).foreach { line =>
            indented.append("    ")
