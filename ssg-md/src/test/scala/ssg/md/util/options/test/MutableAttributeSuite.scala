...
final class MutableAttributeSuite extends munit.FunSuite {
  ...
  test("test_Style") {
    ...
    assertEquals(attribute.getValue(), "color:#white", "add value")

    attribute.setValue("background:#black")
    assertEquals(attribute.getValue(), "color:#white;background:#black", "add value")

    attribute.setValue("font-family:monospaced;color:#green")
    assertEquals(attribute.getValue(), "color:#green;background:#black;font-family:monospaced", "add and change multiple values")

    attribute.setValue("font-family")
    assertEquals(attribute.getValue(), "color:#green;background:#black", "remove value")

    attribute.removeValue("color;background")
    assertEquals(attribute.getValue(), "", "remove values")
  }
}
