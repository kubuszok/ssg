...
final class YamlFrontMatterTest extends munit.FunSuite {
  ...
  /** The visitor's data as the `java.util` map the java test reads, in the visitor's own key order. */
  private def dataOf(visitor: AbstractYamlFrontMatterVisitor): java.util.Map[String, java.util.List[String]] = {
    val data = new java.util.LinkedHashMap[String, java.util.List[String]]()
    visitor.getData().foreach { case (key, values) => data.put(key, values.asJava) }
    data
  }
