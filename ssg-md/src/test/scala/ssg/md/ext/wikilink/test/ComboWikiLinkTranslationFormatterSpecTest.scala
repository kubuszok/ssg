...
final class ComboWikiLinkTranslationFormatterSpecTest extends TranslationFormatterSpecTestSuite {
  ...
  override def defaultOptions:       Nullable[DataHolder]                   = ComboWikiLinkTranslationFormatterSpecTest.OPTIONS
...
object ComboWikiLinkTranslationFormatterSpecTest {
  ...
  val OPTIONS:           DataHolder       =
    new MutableDataSet().set(Parser.EXTENSIONS, Collections.singleton(WikiLinkExtension.create())).set(Parser.UNDERSCORE_DELIMITER_PROCESSOR, java.lang.Boolean.valueOf(false)).toImmutable()
