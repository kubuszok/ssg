...
final class ComboSubscriptSpecTest extends RendererSpecTestSuite {
  ...
  override def defaultOptions: Nullable[DataHolder]                   = ComboSubscriptSpecTest.OPTIONS
...
object ComboSubscriptSpecTest {
  ...
  val OPTIONS: DataHolder = new MutableDataSet().set(HtmlRenderer.INDENT_SIZE, java.lang.Integer.valueOf(0)).set(Parser.EXTENSIONS, Collections.singleton(SubscriptExtension.create())).toImmutable()
