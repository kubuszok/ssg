...
final class FileUriIncludeIss983Suite extends munit.FunSuite {
  ...
  final private class FileLinkResolverFactory extends LinkResolverFactory {
    // null is java's own "no dependents" answer for a Dependent
    override def getAfterDependents():                     mutable.Set[Class[?]] = null
    override def getBeforeDependents():                    mutable.Set[Class[?]] = null
  ...
  test("ISS-983: include resolves a filesystem file:/ path to the file content") {
    ...
    val factories: mutable.Buffer[LinkResolverFactory] = mutable.ArrayBuffer.empty[LinkResolverFactory]
    factories += new FileLinkResolverFactory()
