...
final class MarkdownTableTest extends MarkdownTableTestBase {
  ...
  test("test_LeftAligned_TypedSpaceAfter") {
    ...
    val source = BasedSequence.of(charSequence)
    val table  = getTable(source, formatOptions("", null))
    assert(table.addTrackedOffset(TrackedOffset.track(pos, Character.valueOf(' '), false)))
    val out = new HtmlWriter(0, LineAppendable.F_FORMAT_ALL)
    table.appendTable(out)
    val formattedTable = out.toString(0, 0)
    val offset         = table.getTrackedOffsetIndex(pos)
  ...
  test("test_LeftAligned_BackspaceAfterEmpty") {
    ...
    assert(table.addTrackedOffset(TrackedOffset.track(pos, null, true)))
    val out = new HtmlWriter(0, LineAppendable.F_FORMAT_ALL)
    table.appendTable(out)
    val formattedTable = out.toString(0, 0)
    val offset         = table.getTrackedOffsetIndex(pos)
  ...
  test("test_LeftAligned_Backspace4AfterEmpty") {
    ...
    assert(table.addTrackedOffset(TrackedOffset.track(pos, Character.valueOf(' '), true)))
  ...
  test("test_EmbeddedPipe") {
    ...
    val table  = getTable(
      source,
      formatOptions("", null).toMutable().set(TablesExtension.FORMAT_TABLE_FILL_MISSING_COLUMNS, java.lang.Boolean.valueOf(false))
    )
  ...
  test("test_EmbeddedPipe0") {
    ...
    table.fillMissingColumns(Integer.valueOf(0))
...
//        table.fillMissingColumns(Integer.valueOf(0))
  ...
  test("test_indentPrefix") {
    ...
    val table  = getTable(
      source,
      formatOptions("", null).toMutable().set(TablesExtension.FORMAT_TABLE_FILL_MISSING_COLUMNS, java.lang.Boolean.valueOf(false)).set(TablesExtension.FORMAT_TABLE_INDENT_PREFIX, "    ")
    )
    table.fillMissingColumns()
